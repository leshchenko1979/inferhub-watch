...
REGISTRY = [
    ...
    {"id": "cache_tools", "scores_rank": True},
...
class ScoringSpecsTests(unittest.TestCase):
    def test_filters_to_scores_rank(self):
        ...
        self.assertEqual([s["id"] for s in specs], ["stream_tools", "cache_tools"])


class ProbeRoutesTests(unittest.TestCase):
    def test_sweeps_routes_and_records_errors(self):
        stream = mock.Mock()
        stream.run.side_effect = [_cell("stream_tools"), RuntimeError("boom")]
        cache = mock.Mock()
        cache.run.side_effect = [
            _cell("cache_tools", evidence={"hit_ratio": 0.94}),
            _cell("cache_tools", status="fail", summary="no cache"),
        ]
        modules = {"stream_tools": stream, "cache_tools": cache}
        ...
        for cell in cells:
            self.assertNotEqual(cell["check_id"], "usage_pricing")

    def test_balance_too_low_stops_sweep(self):
        stream = mock.Mock()
        stream.run.side_effect = RuntimeError("balance too low for this request")
        modules = {"stream_tools": stream, "cache_tools": mock.Mock()}
        stderr = io.StringIO()
        with mock.patch.object(
            routes, "load_check_module", side_effect=lambda cid: modules[cid]
        ...
        modules["cache_tools"].run.assert_not_called()
...
class FormatTableTests(unittest.TestCase):
    def test_renders_status_latency_cache_summary(self):
        ...
        self.assertIn("cache_tools", text)
...
class MainTests(unittest.TestCase):
    ...
    def test_sweep_prints_table(self):
        module = mock.Mock()
        module.run.return_value = _cell("stream_tools", summary="tool call streamed")
        with mock.patch.dict(
            "os.environ", {"INFERHUB_API_KEY": "sk-test"}, clear=True
        ), mock.patch.object(routes, "load_registry", return_value=REGISTRY), mock.patch.object(
