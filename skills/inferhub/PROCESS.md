...
- **Prefer topic workers over sub-agents.** Worker = a cron session bound to the worker's forum topic (delivers/reads there), or any lane reachable in-thread. Sub-agents are the fallback for one-shot mechanical edits — and a sub-agent's result is only real after HQ verifies it first-hand (fresh `git pull` + `git log` receipt).
