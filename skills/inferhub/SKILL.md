...
The discovered objects span 8 concrete classes:
...
8. **Forum & Topics:** the Telegram **forum** (group `-1004379632866`, "Inferhub watch") and each **topic** in it are first-class project objects — they are the project's worker lanes. Topic 2 = HQ; the numbered worker topics (created via userbot `messages.CreateForumTopic`, bound in `session_bindings`) are per-issue lanes, renamed `Done — …` when their issue closes (cleanliness law). Topic state is inspectable with `messages.GetForumTopics`.
...
| 3 Surfaces | 2026-09-11 07:5xZ | #15 | filed; basis ruled by owner |
