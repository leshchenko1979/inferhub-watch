...
|---|---|---|---|
| **Topic worker** | ALL implementation, bugfix, sync, research | forum topic + `session_bindings` | spawn prompt is the thin seed; substantive brief via `session_notify` |
| **Review sub-agent** | reviews ONLY (fresh context, no session bias) | none | spawn prompt carries the task |

**Delivery law (two failures, 2026-09-11):** a `session_notify` "delivered" verdict is a ROUTE report, not proof the worker woke. Workers `4f72ca76` and `2bb2e9b9` both missed their brief. When the worker has already completed a turn, the brief MUST go via `resume_agent` / `send_input`. Record which path was used — never write "briefed via `session_notify`" when the fallback carried it.
