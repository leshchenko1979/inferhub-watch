...
- **Cost per M tokens** — board routes only, with the **spend dashboard** at the top of the section: month-to-date cost, today’s cost, and probe’s share, all from [data/pricing.json](data/pricing.json) (30-day usage logs), plus a 30-day sparkline (log-scaled bars, hover for the day total). Ask in/out billed from usage; the **Δ ask** column compares with the previous day’s snapshot ([data/pricing/](data/pricing/), one file per day) — ↓ is cheaper, ↑ pricier. Each route’s plumbing row (the “more” fold) carries ask movement, ask source, cache hit, failures, window traffic and cost, ttft/tps, IQ, and retries.
- **Reading guide** — all explanatory prose lives on the separate [docs page](site/docs.md) (`docs.html`, built from `site/docs.md`): how to read the pricing table and the scatter chart, the Δ ask color key, the results/candidates table, the endpoint, and one explainer per check. The board itself stays numbers-only and links over via "Reading guide" in the nav and one-line captions.
