...
| **Board** | The main pricing table — one row per traffic-carrying route; rows sort by **$/M ascending** (cheapest first — the board answers "what is this costing me") on a **gated basis**: the realized 30-day cost while the projection gate fails, the **projected** cost once it passes. Ties break by **IQ per $** descending; routes without an eff figure last. **This is a deliberate divergence from the North Star metric:** the board sorts by $/M, NOT by Value — it is the cost-transparency surface, while the **Switcher Value Crown** is the North Star optimizer. The divergence is documented here rather than left ambiguous because the projection gate certifies *this* ordering, so a reader must be able to see that the certified ordering and the optimized metric are different things | `site/board.py::_iq_sort_key`, `_board_basis` (`use_proj = gate["pass"]`); `site/generate.py::pricing_section` |
| **Board Cost Crown** | The route the board ranks **first** under that gated basis — the cheapest $/M, with IQ per $ only the tiebreak inside an equal cost band. Answers "what is this costing me" and is the unit the projection gate backtests: for each consecutive snapshot pair, the crowned route's realized cost is compared with the true cheapest realized cost. **A cost crown, not a Value crown** — it sorts by $/M, not by the **North Star metric**, so it can and does name a different route than the Switcher Value Crown. The board's cost-first order stands deliberately; the Value ranking ships as a separate, labelled board column rather than replacing it | `probe/official_compare.py::_crown_regrets`; `site/board.py::_iq_sort_key` |
| **Switcher Value Crown** | The active fleet winner selected by the automated switcher based on empirical Value: `Value = (Coding_IQ / eff_price) * (True_TPS / tps_ref) ** 0.50`, where `eff_price` matches the board crown basis (`probe/basis.py::board_basis` with cold-start fleet prior fallback) after applying orthogonal constraints (Coding IQ floor >= 70.0, tool support, DeepSeek date guard). Swaps fleet route when challenger exceeds incumbent by >15% | `scripts/auto_route_switch.py::find_best_route` |
...
| **Marginal realized** | Realized $/M over usage rows strictly since the previous snapshot — the fair forward comparator the projection gate is INTENDED to be judged against once the North Star rebase lands. NOT what the gate reads today: the gate's comparator is `eff_per_mtok`, the realized 30-day figure, and `marginal` appears nowhere in `probe/official_compare.py`. Realized-over-30d smears price changes across the whole window; this slice does not. Dimmed to probe-only when every fresh request for the route falls inside a sweep window (real money, unrepresentative cache-cold workload) | `pricing.prior_snapshot_cutoff`, `pricing.marginal_stats`, `generate._probe_only` |
...
| **Intelligence (IQ / Coding IQ)** | Artificial Analysis Coding Intelligence Index for a route's model (0–100 scale; fallback ratio 1.85 from general index) — fetched fresh from artificialanalysis.ai every sweep into `data/intelligence.json`; effort level pinned to (max). Bounded by minimum coding floor 70.0 across fleet. Never hardcoded | `probe/intelligence.py`, models.toml `[aa]` slug map |
| **IQ per $** | Intelligence ÷ the route's effective $/M — the smarter-per-dollar verdict column. Higher is better | board `IQ per $` column |
| **TPS Weight** | Empirical throughput multiplier on Value: square-root power-law `(tps / tps_ref) ** 0.50` using production `tps_mean` over a sample-count-driven window — the newest 24h, widened to at most 7 days when that slice holds fewer than `TPS_FLOOR` (5) valid samples — or sustained qualification probe (`probe/tps_qual.py`), normalized against the dynamic fleet median prior `probe/official_compare.py::fleet_tps_prior`. The 5-sample floor is the evidence bar and does NOT move with the window: widening looks further back for the same bar, never lowers it | `scripts/auto_route_switch.py::calculate_value`, `probe/tps_qual.py`, `probe/pricing.py::perf_stats` |
| **North Star metric** | The system's objective: the TPS-adjusted realized ask a route delivers — Value measured on realized outcomes. The predictor forecasts it, the scoreboard scores it, the switcher's crown maximizes it. Never a sort key of the **Board**, which stays cost-first by design (see that row) | `probe/value.py` (the formula's single owner); `scripts/auto_route_switch.py::calculate_value`; `scripts/predictor_scoreboard.py` |
