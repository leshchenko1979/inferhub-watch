...
| **Crown** | The route the board ranks **first** under that gated basis — the cheapest $/M, with IQ per $ only the tiebreak inside an equal cost band. The crown is what the switcher acts on, so it is also the unit the projection gate backtests: for each consecutive snapshot pair, the crowned route's realized cost is compared with the true cheapest realized cost | `probe/official_compare.py::_crown_regrets`; `site/board.py::_iq_sort_key` |
