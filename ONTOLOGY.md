...
| **Projection gate** | The backtest that decides when the forward view is trusted: replay projection(t) vs realized(t+1) across dated snapshots; flips IQ/$ ranking + verdict + board sort onto the projection only when ≥80% of ≥20 transitions land within 20%. Recomputed per render — never hardcoded | `official_compare.projection_gate` |
| **Marginal realized** | Realized $/M over usage rows strictly since the previous snapshot — the fair forward comparator the gate will be judged against once ~2 weeks accumulate; realized-over-30d smears price changes, this slice does not | `pricing.prior_snapshot_cutoff`, `pricing.marginal_stats` |
